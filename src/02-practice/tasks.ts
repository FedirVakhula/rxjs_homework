import { Observable, of, from, fromEvent, generate, pairs, EMPTY, concat, timer, zip, range, bindCallback, bindNodeCallback, fromEventPattern, interval, NEVER, throwError, defer, forkJoin } from "rxjs";
import { map, take, tap, switchMap, filter, reduce, catchError, delay, concatMap, withLatestFrom, debounceTime, distinctUntilChanged, pluck } from "rxjs/operators";
...
// потрібно зраєструватися для роботи з цим апі, реєстрація не складна, в два кліки, якщо будуть якісь питання - пишіть )

(function tasck1() {

  const inputElement = document.getElementById('text-field');
  const inputStream$ = fromEvent(inputElement, 'input')
    .pipe(
      map(e => {
      ...
      switchMap(searchTerm =>
        ajax({
          url: `https://imdb8.p.rapidapi.com/title/auto-complete?q=${searchTerm}`,
          headers: {
            'Access-Control-Allow-Origin': '*',
            "x-rapidapi-key": "3a3a5830b4mshc617773d934ff3ap1e66d8jsna16385f1e635",
...
(function tasck2() {
  ...
  const inputStream$ = fromEvent(inputElement, 'input')
    .pipe(
      ...
      pluck('d'),
      switchMap((arr) => from(arr)),
      concatMap((obj: any) => forkJoin([
        ajax({
          url: `https://imdb8.p.rapidapi.com/title/get-genres?tconst=${obj.id}`,
          headers: {
            'Access-Control-Allow-Origin': '*',
            "x-rapidapi-key": "3a3a5830b4mshc617773d934ff3ap1e66d8jsna16385f1e635",
            "x-rapidapi-host": "imdb8.p.rapidapi.com"
      ...
      map(([genres, name]) => `Film name: ${name}, ganres: ${genres}`)
    );
  // run(inputStream$);
...
(function tasck3() {
  ...
  const selectStream$ = fromEvent(selectGenres, 'change').pipe(
    map(e => {
      return (e.target as HTMLInputElement).value
    }));
  const inputStream$ = fromEvent(inputElement, 'input')
    .pipe(
      ...
      withLatestFrom(selectStream$),
      filter(([[genres], selectedGenres]) => genres.includes(selectedGenres)),
      map(([data]) => data),
      map(([genres, name]) => `Film name: ${name}, ganres: ${genres}`)
    );
  // run(inputStream$);
